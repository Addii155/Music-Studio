...
const ResponsiveNavbar = () => {
  ...
  return (
    <div className={`w-full mb-2 flex justify-between items-center font-semibold ${theme === "light" ? "bg-white text-black" : "bg-black text-white"}`}>
      ...
      <Sheet open={isMenuOpen || isCreatingPlaylist} onOpenChange={setIsMenuOpen}>
        <SheetContent side="left" className="bg-[#282828] text-white">
          <SheetHeader>
            <SheetTitle>{isCreatingPlaylist ? "Create Playlist" : "Menu"}</SheetTitle>
            {isCreatingPlaylist && (
              <SheetDescription>
          ...
          {!isCreatingPlaylist ? (
            <div className="flex flex-col gap-4">
              {/* Home Link */}
              <div
                className={`flex items-center gap-2 pl-4 cursor-pointer ${location.pathname === "/" && "text-black bg-white rounded-2xl"}`}
                onClick={() => navigate("/")}
              >
              ...
              <div
                className={`flex items-center gap-3 pl-4 cursor-pointer ${location.pathname === "/favourite" && "text-black bg-white rounded-2xl"}`}
              ...
              </div>
              {
                user ? (
                  <div
                className="flex items-center my-2 text-xl hover:text-slate-300 justify-center  items-center  cursor-pointer" 
                >
                ...
                : (
                  <div
                className="flex items-center justify-center my-2 text-xl  hover:text-slate-300   cursor-pointer"
                >
                    <button
                    className="text-black bg-white rounded-2xl px-6"
                    >
